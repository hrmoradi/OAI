...
def retrieve_and_save_mri(pid, cohort, time, visit_id, mri_barcode, save_dir, filename):
    slices = []
...
def main():
    ...
    for i, row in filtered_patients.iterrows():
        ...
        retrieve_and_save_mri(row["ID"], cohort, date, visit_id, str(int(row["V00MRBARCD"]))[3:], args.outdir, filename) # Remove the 166 at beginning to get id
